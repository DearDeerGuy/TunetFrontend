...
export const updateFailFilm = async (film_id,{file},token) => {
    const formData = new FormData();
    formData.append("film_id", film_id);
    formData.append("file", file);
    const response = await axios.post(`http://localhost:80${dopPatch}/api/file/update/${film_id}`,formData,{headers: {Authorization: `Bearer ${token}`}});
    return response.data;
}

export const updateFailSerial = async (film_id,{file,season_number,episode_number},token) => {
    const formData = new FormData();
    formData.append("film_id", film_id);
    formData.append("file", file);
    formData.append("season_number", season_number);
    formData.append("episode_number", episode_number);
    const response = await axios.post(`http://localhost:80${dopPatch}/api/file/update/serial/${film_id}`,formData,{headers: {Authorization: `Bearer ${token}`}});
    return response.data;
}

export const deleteFailFilm = async (film_id,token) => {
    const formData = new FormData();
    formData.append("film_id", film_id);
    const response = await axios.delete(`http://localhost:80${dopPatch}/api/file/delete/${film_id}`,formData,{headers: {Authorization: `Bearer ${token}`}});
    return response.data;
}

export const deleteFailSerial = async (film_id,{season_number,episode_number},token) => {
    const formData = new FormData();
    formData.append("film_id", film_id);
    formData.append("season_number", season_number);
    formData.append("episode_number", episode_number);
