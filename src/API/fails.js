...
export const getVideoStream = async (link,token) => {
    const response = await axios.get(`http://localhost:80${dopPatch}/api/stream/${link}`,
        {headers: {
...
export const addFail = async (film_id,{file,season_number,episode_number},token) => {
    const formData = new FormData();
    formData.append("films_id", film_id);
...
export const deleteFail= async (id,token) => {
    const response = await axios.delete(`http://localhost:80${dopPatch}/api/file/${id}`,{headers: {Authorization: `Bearer ${token}`}});
    return response.data;
}